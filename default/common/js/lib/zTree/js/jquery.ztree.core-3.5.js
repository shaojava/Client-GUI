...
(function($) {
    ...
    view = {
        addNodes: function(setting, parentNode, newNodes, isSilent, isPrepend) {
            ...
            if (parentNode) {
                ...
                view.createNodes(setting, parentNode.level + 1, newNodes, parentNode, isPrepend);
                if (!isSilent) {
                    view.expandCollapseParentNode(setting, parentNode, true);
                }
            } else {
                data.addNodesData(setting, data.getRoot(setting), newNodes);
                view.createNodes(setting, 0, newNodes, null, isPrepend);
            }
        },
        ...
        asyncNode: function(setting, node, isSilent, callback) {
            ...
            $.ajax({
                ...
                headers:tools.apply(setting.async.headers, [setting.treeId, node], setting.async.headers),
        ...
        createNodes: function(setting, level, nodes, parentNode, isPrepend) {
        ...
        makeDOMNodeMainBefore: function(html, setting, node) {
            html.push("<li id='", node.tId, "' class='", consts.className.LEVEL, node.level, " ", node.className, "' tabindex='0' hidefocus='true' treenode");
            html.push(">");
            html.push("<div class='item file_item'");
            if (node.level > 0) {
                html.push(" style='padding-left:" + node.level * 30 + "px'");
            }
            for (var key in node) {
                if (String(key).indexOf('data-') === 0 && node.hasOwnProperty(key)) {
                    html.push(" " + key + "='" + node[key] + "'");
                }
            }
            html.push(">");
        },
        ...
        makeDOMNodeNameBefore: function(html, setting, node) {
            ...
            html.push("<span id='", node.tId, consts.id.A, "' class='", consts.className.LEVEL_NAME, ' ', consts.className.LEVEL, node.level, "' treeNode", consts.id.A, " onclick=\"", (node.click || ''),
                    "\" ", ((url != null && url.length > 0) ? "href='" + url + "'" : ""), " target='", view.makeNodeTarget(node), "' style='", fontStyle.join(''),
                    "'");
            if (tools.apply(setting.view.showTitle, [setting.treeId, node], setting.view.showTitle) && title && !setting.view.nameIsHTML) {
    ...
    $.fn.zTree = {
        ...
        init: function(obj, zSetting, zNodes) {
            ...
            var zTreeTools = {
                setting: setting,
                addNodes: function(parentNode, newNodes, isSilent, ajaxNodes, isPrepend) {
                    ajaxNodes = arguments[3] === undefined ? true : ajaxNodes;
                    isPrepend = arguments[4] === undefined ? false : isPrepend;
                    if (!newNodes)
                        return null;
                    if (!parentNode)
                        parentNode = null;
                    ...
                    function addCallback() {
                        view.addNodes(setting, parentNode, xNewNodes, (isSilent == true), isPrepend);
                    }

                    if (ajaxNodes && tools.canAsync(setting, parentNode)) {
